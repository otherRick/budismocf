    ...
    <div className='space-y-8'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        ...
        <div className='bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6'>
          <h2 className='text-xl font-semibold text-white mb-4'>Visitantes por Dia</h2>
          <div className='h-64 flex items-end justify-between space-x-2'>
            {analyticsData.visitorsByDay.map((day) => (
              <div key={day.day} className='flex flex-col items-center flex-1'>
                ...
                />
                <div className='mt-2 text-gray-400 text-sm'>{day.day}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Top Pages */}
        <div className='bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6'>
          <h2 className='text-xl font-semibold text-white mb-4'>Páginas Mais Visitadas</h2>
              ...
              <div key={page.path} className='flex items-center justify-between'>
                <div className='flex items-center'>
                  ...
                  <div>
                    ...
                    <div className='text-gray-400 text-sm'>{page.path}</div>
                  </div>
                </div>
                <div className='text-amber-400 font-medium'>{page.views} views</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Top Sources */}
        <div className='bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6'>
          <h2 className='text-xl font-semibold text-white mb-4'>Principais Fontes</h2>
          <div className='space-y-4'>
            {analyticsData.topSources.map((source) => (
              <div key={source.name} className='flex items-center justify-between'>
                ...
                <div className='text-amber-400 font-medium'>{source.visitors} visitas</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Top Countries */}
        <div className='bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6'>
          <h2 className='text-xl font-semibold text-white mb-4'>Principais Países</h2>
          <div className='space-y-4'>
            {analyticsData.topCountries.map((country) => (
              <div key={country.name} className='flex items-center justify-between'>
                ...
                <div className='flex items-center'>
                  ...
                  <div className='text-amber-400 font-medium'>{country.visitors}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className='bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6'>
          ...
          <div className='flex space-x-3'>
            <a
              ...
              className='inline-block px-4 py-2 bg-black text-white rounded-lg hover:bg-gray-900 transition-colors'
            >
